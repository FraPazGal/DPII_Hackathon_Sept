...
public interface RoomRepository extends JpaRepository<Room, Integer> {
	...
	@Query("select r from Room r where r.visibility = 'DRAFT' and r.owner.id = ?1")
	Collection<Room> findRoomsDraftAndMine(Integer ownerId);
	
	@Query("select r from Room r where r.visibility = 'ACTIVE' and r.owner.id = ?1")
	Collection<Room> findRoomsActiveAndMine(Integer ownerId);
	
	@Query("select r from Room r where r.visibility = 'OUTOFSERVICE' and r.owner.id = ?1")
